...
*Condition met 2026-09-13.* `udapcfg discover` found a Squeezebox in setup mode
(`00:04:20:16:17:18`) on macOS 25.6.0 and on Linux 6.18.42, matching go-udap in
both cases. Task 6's checklist records what passed and what did not: steps 1-6
and 8 pass, step 7 (Windows at runtime) and step 9 (a link failing mid-discovery)
remain open, and the run found one fidelity bug — non-deterministic `interfaces`
row order on Linux, [issue #13](https://github.com/yo61/udapcfg-rs/issues/13).
The Windows `#[cfg]` arm named in this milestone's title is compiled and linted
by CI but is still never invoked by a test.
...
| No privilege warning in the `--bind-interface` failure path, where go-udap's text says binding "may require `CAP_NET_RAW`" | None observable on a current kernel. Measured 2026-09-13 on Linux 6.18.42 as an unprivileged user: `SO_BINDTOIFINDEX` bound and discovery succeeded, and go-udap's `SO_BINDTODEVICE` was equally unrestricted there — so the warning is not reproducible for *either* implementation and reproducing it would mean copying advice that no longer applies. Untested below 6.18.42; if an older kernel does restrict `SO_BINDTODEVICE` while `SO_BINDTOIFINDEX` stays open, this becomes a case where the Rust port is *less* restrictive than the Go. See [OQ-2](#open-questions) |
...
| `udapcfg interfaces` sorts by name: `en2` before `en10`, VLAN sub-interfaces by id (`eth0.99` before `eth0.100`), and opaque hex ids such as docker's `br-a27c6b90d180` in hex order; go-udap prints whatever order the OS returns | **User-visible row order differs.** Forced: `netdev` guarantees no order, and its Linux backend collects through a `HashMap`, so Rust's per-process `RandomState` made the table print differently on every run ([#13](https://github.com/yo61/udapcfg-rs/issues/13)). Matching go-udap exactly is not possible with one rule — the OS order it prints is ascending index on Linux but creation order on macOS, where `en0` (index 15) precedes `en8` (index 13). A stable, explainable order was judged better than a random one. Applied in `enumerate()`, so the table, `--bind-interface` validation and `--all-interfaces` all agree. Row *content* is unchanged |
...
*Verified at M3 (2026-09-13): `flags` is populated with default features off.*
Confirmed rather than assumed, against a live Tailscale interface: `utun16` is
up, is not a loopback, and carries IPv4 `100.122.155.106`, yet `is_broadcast()`
returns `false` and `enumerate()` excludes it on the broadcast test alone. The
flags discriminate — `en0`/`en8` report `true` in the same run — so they are not
defaulting. This is also the regression the carried finding warns about: delete
the `IFF_BROADCAST` check and discovery would fan out into a VPN tunnel.

**OQ-2 — `SO_BINDTOIFINDEX` privileges. RESOLVED: no privileges needed.**
The question was whether `SO_BINDTOIFINDEX` carries the `CAP_NET_RAW`
requirement that go-udap's error message attributes to `SO_BINDTODEVICE`, since
a difference would change fidelity-contract error text.

Measured on 2026-09-13 against a Squeezebox in setup mode, from a TrueNAS Scale
host (Linux 6.18.42, x86_64) as an unprivileged user:
`udapcfg --bind-interface bond0 discover` bound the socket and found the device,
exit 0. No `EPERM`. go-udap, which uses `SO_BINDTODEVICE`, was equally
unrestricted on the same kernel — so neither option is privileged there, and the
Rust port is **not** more restrictive than the Go.

**Consequence:** the error text needs no privilege warning. go-udap's claim that
binding "may require CAP_NET_RAW" is not reproducible on a current kernel.

*Not established:* whether an older kernel restricts `SO_BINDTODEVICE` where
`SO_BINDTOIFINDEX` would not. Only 6.18.42 was available, well above the 5.7
floor that the accepted-deltas table already records as a narrowing.
